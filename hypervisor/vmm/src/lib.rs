...
impl Vmm {
    ...
    fn vm_pause_to_snapshot(
        ...
        // Live-connection freeze: connections intentionally cross the
        // freeze alive; the guest is told only after restore, via
        // RST-on-restore (Vsock::new -> queue_rst_for_connections, from
        // Cloud Hypervisor #7958). Log the count so the invariant and any
        // deviation from it are visible in the logs.
        if let Some(vm) = self.vm.as_ref() {
            info!(
                "vsock connections at freeze: {} ts={}",
                vm.vsock_conn_count(),
