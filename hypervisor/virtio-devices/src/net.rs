...
impl VirtioDevice for Net {
    fn stop_workers(&mut self) {
        // Clear paused and signal kill before unparking the ctrl worker:
        // an unpark issued while paused is still true can be consumed by
        // a re-park and lost, leaving the join below blocked forever.
        self.common.stop_workers();
        if let Some(ctrl) = &self.ctrl_queue_epoll_thread {
            ctrl.thread().unpark();
        }
